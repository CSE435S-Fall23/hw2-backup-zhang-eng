...
public class HeapPage {
	 ...
	 * 
	 * Number of records in a heappage 
	 * 
	 * @return number of slots on this page
	 */
	public int getNumSlots() {
		return (int)((HeapFile.PAGE_SIZE*8.0)/((td.getSize()*8.0)+1));
	}
	...
	public boolean slotOccupied(int s) {
		...
		if((byte) ((header[index] >> shift) & 0x1) == 0) {
			return false;
		}
		return true;
	...
	public void setSlotOccupied(int s, boolean value) {
		...
		int shift = s % 8;
		if (value) {
			header[index] = (byte) (header[index] | (1 << (shift)));
		}
		else {
			header[index] = (byte) (header[index] & ~(1 << (shift)));
		}
	...
	public void addTuple(Tuple t) throws Exception {		
		...
		throw new Exception("Tuple count not be inserted: Either no empty slots are available or tuple does not have the correct structure.");
	}
